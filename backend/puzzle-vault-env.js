...
/**
 * Full vault-related env when PUZZLE_SOURCE=sqlite. Throws if required vars are missing/invalid.
...
export function requireSqliteVaultEnv() {
  if (parsePuzzleSource() !== PUZZLE_SOURCE_SQLITE) {
    throw new Error("requireSqliteVaultEnv() only valid when PUZZLE_SOURCE=sqlite")
  }
...
export function puzzleVaultEnvSummary() {
  try {
    ...
    return {
      ...
      sqlite_path: v.sqlitePath,
      backup_dir: v.backupDir,
      backup_keep: v.backupKeep,
      quest_mint: v.questMint,
      quest_fund_amount_raw: v.questFundAmountRaw,
      hkdf_salt_configured: Boolean(v.hkdfSaltHex),
