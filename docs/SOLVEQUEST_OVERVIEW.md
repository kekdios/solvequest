...
- **Product surface**: landing, email OTP auth, in-app **Quick start** (sidebar), **perpetual-style trading UI** (**Trade** screen; Hyperliquid-derived index marks), **QUSD** balances, **history** of closed perps, **buy QUSD** (Solana USDC → QUSD crediting via verified receive address + server scan), **Swap** (QUSD → USDC from treasury; **`plugins/swapApiPlugin.ts`**), **Prize** (daily pool amount from env; public **`GET /api/prize/config`**), **Leaderboard** (public **`GET /api/leaderboard`** — top accounts by ledger QUSD, masked email), optional **Visitors** (admin), and account settings.
...
| Prize (display) | **`PRIZE_AMOUNT`** — QUSD amount for landing and **Prize** screen; exposed publicly via **`GET /api/prize/config`**. Automatic daily award: **`server/dailyPrizeScheduler.ts`** runs **`0 16 * * *`** in **`America/New_York`** (4:00 PM Eastern) and credits **`PRIZE_AMOUNT`** to the top **prize-eligible** leaderboard account (see **`server/dailyPrizeAward.ts`**). Set **`SOLVEQUEST_DISABLE_DAILY_PRIZE_AWARD=1`** to disable. Recent winners: **`GET /api/prize/awards`**. |
| Swap QUSD→USDC | **`SWAP_ABOVE_AMOUNT`** (QUSD **floor**: gross entered amount must exceed this; **effective** QUSD that converts = `min(entered, balance) − floor`), **`SWAP_QUSD_USDC_RATE`** (QUSD per 1 USDC), **`SWAP_MAXIMUM_USDC_AMOUNT`**, optional **`SWAP_USDC_RECEIVE_ADDRESS`**; treasury **`SOLANA_TREASURY_*`** — see **`plugins/swapApiPlugin.ts`** and **`src/lib/swapAmounts.ts`**. |
...
- **Swap (QUSD → USDC)**: **`plugins/swapApiPlugin.ts`** exposes swap config, preflight, and **`POST /api/swap`** — debits QUSD from the ledger and sends **USDC** from the treasury to the user’s verified Solana receive address (see **`server/usdcSwapTransfer.ts`**). The treasury must hold **USDC** and **SOL** for fees; first send may create the user’s USDC ATA (treasury pays rent). **Amount logic:** the user submits a **gross** QUSD amount (must be **greater than** **`SWAP_ABOVE_AMOUNT`**). The server computes **effective QUSD** = `max(0, min(gross, spendable balance) − SWAP_ABOVE_AMOUNT)`. **USDC out** = effective QUSD ÷ **`SWAP_QUSD_USDC_RATE`** (QUSD per 1 USDC), rounded to two decimals, then capped by **`SWAP_MAXIMUM_USDC_AMOUNT`** and treasury USDC on hand — same math as **`computeSwapAmounts`** in **`src/lib/swapAmounts.ts`** (shared with the Swap UI).
- **Prize pool & awards**: **`plugins/prizeConfigApiPlugin.ts`** — public **`GET /api/prize/config`**: `prize_amount` (from **`PRIZE_AMOUNT`**), **`next_award_at_ms`** (next 4:00 PM **`America/New_York`** via **`server/nextDailyPrizeAwardMs.ts`**), **`award_timezone`**, **`award_schedule_label`**. **`GET /api/prize/awards`** returns recent rows from **`daily_prize_award_log`**. **`daily_prize_award_log`** stores **`award_day_est`**, **`awarded_at`**, **`prize_amount`**, **`winner_label`** per Eastern day.
...
- **Public APIs** (no JWT): **`GET /api/prize/config`** (prize pool display number), **`GET /api/prize/awards`** (recent automatic daily prize winners), **`GET /api/leaderboard`** (top QUSD totals; optional cookie highlights the current user’s row), swap **`GET /api/swap/config`** (when configured).
