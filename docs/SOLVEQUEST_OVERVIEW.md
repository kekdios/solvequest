...
- **Prize pool & awards**: **`plugins/prizeConfigApiPlugin.ts`** — public **`GET /api/prize/config`** (`prize_amount` from **`PRIZE_AMOUNT`**) and **`GET /api/prize/awards`** (recent rows from **`daily_prize_award_log`**). **`daily_prize_award_log`** stores **`award_day_est`**, **`awarded_at`**, **`prize_amount`**, **`winner_label`** per Eastern day.
