...
- **Sell QUSD**: **`plugins/qusdSellApiPlugin.ts`** exposes **`GET /api/qusd/sell/config`**, **`GET /api/qusd/sell/me`**, **`POST /api/qusd/sell`** — debits QUSD from the ledger and sends QUEST from treasury to the verified receive address (see **`server/qusdSellTransfer.ts`**).
